...
C-----LIST ENERGY BOUNDARIES.
      ...
      DO 10 III=I,II
      J=J+1
      CALL OUT9(EGROUP(III),FIELDX(1,J))
   10 CONTINUE
      WRITE(OUTP,60) ((FIELDX(M,JJ),M=1,11),JJ=1,J)
      WRITE(*   ,60) ((FIELDX(M,JJ),M=1,11),JJ=1,J)
   20 CONTINUE
...
C-----LIST SPECTRUM.
      ...
      J=J+1
      CALL OUT9(YTAB(III,3),FIELDX(1,J))
   20 CONTINUE
      WRITE(OUTP,160) ((FIELDX(M,JJ),M=1,11),JJ=1,J)
      WRITE(*   ,160) ((FIELDX(M,JJ),M=1,11),JJ=1,J)
   30 CONTINUE
...
c-----------------------------------------------------------------------
   30 SUM=ZERO
      DO 40 I=2,IPT
      SUM=SUM+(XTAB(I)-XTAB(I-1))*(YTAB(I)+YTAB(I-1))
   40 CONTINUE
      SUM=SUM/TWO
      DO 50 I=1,IPT
      YTAB(I)=YTAB(I)/SUM
